...
class MoveNetThunderHPE(object):
    """Class for processing images using MoveNet Thunder"""
    def __init__(self):
        # processing module
        self._module = module = hub.load("https://tfhub.dev/google/movenet/singlepose/thunder/4")
        self._model = self._module.signatures["serving_default"]
        self._input_size = 256

        # Define the mapping of keypoints to body parts
        self._keypoint_names = ['nose', 'left_eye', 'right_eye', 'left_ear', 'right_ear', 'left_shoulder', 'right_shoulder',
                        'left_elbow', 'right_elbow', 'left_wrist', 'right_wrist', 'left_hip', 'right_hip',
                        'left_knee', 'right_knee', 'left_ankle', 'right_ankle']

        # Define the connections between keypoints to draw lines for visualization
        self._connections = [(0, 1), (0, 2), (1, 3), (2, 4), (0, 5), (0, 6), (5, 7), (7, 9), (6, 8), (8, 10),
                    (5, 6), (5, 11), (6, 12), (11, 12), (11, 13), (13, 15), (12, 14), (14, 16)]

    def process_image(self, image):
        """Processes opencv image and returns array of keypoints"""
        
        # convert image to rgb
        image_rgb = cv2.cvtColor(image, cv2.COLOR_BGR2RGB)

        # resize image
        image_resized = tf.image.resize_with_pad(tf.expand_dims(image_rgb, axis=0), self._input_size, self._input_size)
        ...
        return self.keypoints_helper(image, keypoints_with_scores)

    def keypoints_helper(image, keypoints):
        """Converts keypoints to image coordinates"""
        xs, ys, _ = image.shape
        ...
        if xs > ys:
            p = (xs - ys) / 2
            kps = np.array([[kp[1] * xs - p, kp[0] * xs] for kp in kps])

        elif xs < ys:
            p = (ys - xs) / 2
            kps = np.array([[kp[1] * ys, kp[0] * ys - p] for kp in kps])
        else:
            kps = np.array([[kp[1] * xs, kp[0] * ys] for kp in kps])
